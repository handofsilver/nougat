...
class Attention(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None, training_window_size=None):
        ...
        qkv = qkv.reshape(B, N, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4) # 3 B num_heads N out_dim
    ...
    def forward_features(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x, rel_pos_bias, training_window_size, use_reentrant=True)
            else:
