...
# 定义一个函数将文本中用转义符描述的数学符号尽可能还原，例如希腊字母、求和、积分、逻辑符号等
...
def clear_semantic_symbol(text):
    ...
    功能介绍
        r"\^(-?[0-9]+)" 这个模式会匹配：
            ...
            后面跟着可选的负号 -（由 -? 表示）
            一个或多个数字（由 [0-9]+ 表示）
        然后将其替换为 \\1，也就是第一个捕获组的内容（数字部分，包括负号）。
        text = "x^2"      # 变成 "x2"
        text = "10^-3"    # 变成 "10-3"
        text = "a^123"    # 变成 "a123"
    """
...
# 定义一个函数将md中的公式尽可能转换为编译后的文本
def encode_formula_in_md(doc, debug=False):
    """
    1. 根据转义方括号或圆括号提取出doc中的所有公式，公式样例如下：
        单独成行的公式：\[\sum_{k\in k}y_{w,k}\leq m\]
        在正文中的公式：\(y_{i,k}\)
...
def test_encode_formula_in_md():
    base_path = "/home/ninziwei/projects/nougat_ocr/nougat/dataset/test"
    md_path = f"{base_path}/2402.00041.mmd"
    md = open(md_path, "r", encoding="utf-8").read().replace("\xa0", " ")
    # lines = md.split("\n")
    # for line in lines:
    #     if 'heta_{i},x_{j},y_{j}' in line:
    #         print(repr(line))
    #         print('\theta' in line)
    #         print(encode_formula_in_md(line))

    encode_formula_in_md(md, debug=True)


if __name__ == "__main__":

    # text = '\[\mathfrak{s}_{i,j}^{s}=f(x_{i},y_{i},\theta_{i},x_{j},y_{j},\theta_{j})\] (10)'
    # tmp = encode_math_symbol(text)
    # print('转义符还原后：', tmp)
    # tmp = clear_semantic_symbol(tmp)
    # print('公式还原后：', tmp)
