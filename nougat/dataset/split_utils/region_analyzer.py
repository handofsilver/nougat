...
def get_doc_indices_to_regions(
    index_mappings: List[Dict], duplicate_indices: List[int]
) -> Dict[int, List[ContinuousRegion]]:
    """
    分析重复索引的连续区域

    Args:
        index_mappings: page_splitter中的valid_index_mappings输出结果
    ...
    for doc_line_index in duplicate_indices:
        regions = _analyze_single_index_regions(index_mappings, doc_line_index)
        results[doc_line_index] = regions

    return results


def _analyze_single_index_regions(
    index_mappings: List[Dict], doc_line_index: int
) -> List[ContinuousRegion]:
    """分析单个索引的连续区域"""
    # 收集该索引的所有映射
    mappings_by_doc_index = _collect_index_mappings(index_mappings, doc_line_index)
    ...
    for mapping in mappings_by_doc_index:
        ...
        # 判断是否需要开始新区域
        should_start_new_region = _should_start_new_region(
            index_mappings,
            current_page,
            last_pdf_line,
            page_index,
        ...
        if should_start_new_region:
            ...
            if current_region_mappings:
                region = _create_region(len(regions) + 1, current_region_mappings)
...
def _collect_index_mappings(index_mappings, doc_line_index: int) -> List[Dict]:
    """收集指定索引的所有映射"""
    collected_mappings = []

    for page_idx, page_mappings in enumerate(index_mappings):
...
def _should_start_new_region(
    ...
    if pdf_line > last_pdf_line + 1:
        ...
        has_unordered_content = _has_unordered_content_in_gap(
            index_mappings, page_idx, gap_start, gap_end
        )
...
def _has_unordered_content_in_gap(
    index_mappings: List[Dict], page_idx: int, gap_start: int, gap_end: int
) -> bool:
    """检查PDF行间隔中是否有unordered内容"""
    for mapping in index_mappings[page_idx]:
...
def _create_region(region_id: int, mappings: List[Dict]) -> ContinuousRegion:
    """创建连续区域对象"""
